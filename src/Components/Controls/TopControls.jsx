...
const TopControls = ({ onApply, setFadeIn, faculties = [] }) => {
  ...
  useEffect(() => {
    ...
    const cachedAvgTextbookCost = sessionStorage.getItem(
      "cachedAvgTextbookCost"
    );
  ...
  const handleApply = async (
    ...
    try {
      ...
      console.error(error);
      setError(
        "We ran into an error fetching stats, please try again in a few moments."
      );
    } finally {
      setFadeIn(true);
    }
  ...
  const listenForProgress = async (clientId, start, end, avgCost) => {
    const eventSource = new EventSource(
      `http://localhost:8384/get-stats-progress/${clientId}`
    );
      ...
      <Modal open={localLoading}>
        ...
        >
          ...
          >
            <img
              className="logo"
              src="../assets/Images/oer_logo.png"
              alt="OER Logo"
            />
